<template>
  <div>
    <basic-crud
      ...
      showEdit
    />
    <add-dialog @closeDialog="onCloseDialog" ref="addDialog" entidad="Bebidas">
...
export default {
  ...
  methods: {
    ...
    getBebidasIndex(){
      let response = this.allCategorias.find( element => {
        if(element.nombre == 'bebidas' || element.nombre == 'Bebidas'){
          return element
        }
      })
      this.bebidaCategoria = response.idCategoria
      this.editedItem.categoriaId = this.bebidaCategoria
      console.log(this.bebidaCategoria)
    },
